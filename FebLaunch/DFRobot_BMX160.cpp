...
void DFRobot_BMX160::getAllData(struct bmx160SensorData *magn, struct bmx160SensorData *gyro, struct bmx160SensorData *accel){
    ...
    if(gyro){
        ...
        gyro->y = ((int16_t) ((data[11] << 8) | data[10])) * gyroRange;
        gyro->z = ((int16_t) ((data[13] << 8) | data[12])) * gyroRange;
        // gyro->x *= gyroRange;
        // gyro->y *= gyroRange;
        // gyro->z *= gyroRange;
    }
    if(accel){
        accel->x = ((int16_t) ((data[15] << 8) | data[14])) * accelRange;
        accel->y = ((int16_t) ((data[17] << 8) | data[16])) * accelRange;
        accel->z = ((int16_t) ((data[19] << 8) | data[18])) * accelRange;
        // accel->x *= accelRange;
        // accel->y *= accelRange;
        // accel->z *= accelRange;
    }
