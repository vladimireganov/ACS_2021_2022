...
int8_t user_i2c_write(int i2c_bus, uint8_t dev_id, uint8_t reg_addr,uint8_t *data, uint16_t len){
    if (ioctl(i2c_bus, I2C_SLAVE, dev_id) < 0) {
    ...
    for(uint16_t i = 0; i < len; i ++) {
        ...
        if (i2c_smbus_write_byte_data(i2c_bus, reg_addr, data[i]) < 0) {
...
int8_t user_i2c_read(int i2c_bus, uint8_t dev_id, uint8_t reg_addr,uint8_t *data, uint16_t len) {
    int i = 0;
    ...
    for (uint16_t i = 0; i < len; i++) {
        ...
        data[i] = i2c_smbus_read_byte_data(i2c_bus, reg_addr);
